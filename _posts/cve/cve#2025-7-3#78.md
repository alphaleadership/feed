...
CVE ID : CVE-2025-38166 Published : July 3
2025
9:15 a.m. | 2 hours
...
e.g.
when the BPF program executes bpf_msg_push_data(). If the BPF program sets cork_bytes and sg.size is smaller than cork_bytes
it will return -ENOSPC and attempt to roll back to the non-zero copy logic. However
during rollback
msg->msg_iter is reset
but since msg_pl->sg.size has been increased
subsequent executions will exceed the actual size of msg_iter. ''' iov_iter_revert(&msg->msg_iter
msg_pl->sg.size - orig_size); ''' The changes in this commit are based on the following considerations: 1. When cork_bytes is set
