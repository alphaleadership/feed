...
CVE ID : CVE-2025-38150 Published : July 3
2025
...
which may require disabling promiscuous and/or allmulti mode. Both of these operations require acquiring the netdev instance lock. Move the call to `packet_dev_mc` outside of the RCU critical section. The `mclist` modifications (add
del
flush
unregister) are protected by the RTNL
not the RCU. The RCU only protects the `sklist` and its associated `sks`. The delayed operation on the `mclist` entry remains within the RTNL. Severity: 0.0 | NA Visit the link for more details
