...
CVE ID : CVE-2025-38151 Published : July 3
2025
9:15 a.m. | 2 hours
...
which corrupted the work item currently in the work queue. However
it left a problem when queue_work fails (because the item is still pending in the work queue from a previous call). In this case
cma_id_put (which is called in the work handler) is therefore not called. This results in a userspace process hang (zombie process). Fix this by calling cma_id_put() if queue_work fails. Severity: 0.0 | NA Visit the link for more details
