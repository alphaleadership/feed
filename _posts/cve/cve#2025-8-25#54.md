...
In addition to values of `len` greater than 130 triggering a buffer overflow
a value of `len` smaller than 2 will also trigger a buffer overflow due to an integer underflow when computing `len-2` in this code path.
