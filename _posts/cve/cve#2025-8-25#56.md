...
date: 2025-8-25
lien: "https://cvefeed.io/vuln/detail/CVE-2025-54489"
categories:
  - cve
---
...
hdr);
                        if (VERBOSE_LEVEL==9)
                            fprintf(stdout
"MFER: tag=%3i chan=%2i len=%-4i tag2=%3i len2=%3i curPos=%i %li count=%4i\n"
tag
chan
len
tag2
...
Here
the number of bytes read is not the Data Length decoded from the current frame in the file (`len`) but rather is a new length contained in a single octet read from the same input file (`len2`). Despite this
a stack-based buffer overflow condition can still occur
as the destination buffer is still `buf`
which has a size of only 128 bytes
while `len2` can be as large as 255.
